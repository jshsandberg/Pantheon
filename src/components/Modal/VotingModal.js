...
export default function VotingModal({ show, hide, item, pantheon, user, number, rerenderPage }) {
    ...
    const vote = async (username, fighter, pantheonId, battle, number) => {
        await SubmitVote(username, fighter, pantheonId, battle, number);
        await rerenderPage()
        await onClose();
    }
    ...
    return (
        <div className="votingModal">
            ...
            <div style={{display: "flex", flexDirection: "row", justifyContent: "space-around"}}>
                <button onClick={() => vote(user.username, "fighterOne", pantheon._id, item, number)} style={{backgroundColor: "dodgerblue", borderColor: "dodgerblue", fontSize: "20px", padding: "20px"}}>Vote for {item.fighterOne.username}</button>
                <h2 style={{textAlign: "center", padding: "20px 0px 0px 0px"}}>VS</h2>
                <button onClick={() => vote(user.username, "fighterTwo", pantheon._id, item, number)} style={{backgroundColor: "dodgerblue", borderColor: "dodgerblue", fontSize: "20px", padding: "20px"}}>Vote for {item.fighterTwo.username}</button>
            </div>
